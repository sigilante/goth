...
pub enum Token {
    ...
    // ============ Imaginary Literals ============
    #[regex(r"[0-9]+(\.[0-9]+([eE][+-]?[0-9]+)?)?\u{1D55A}", priority = 3, callback = |lex| {
        let s = lex.slice();
        let num = &s[..s.len() - "\u{1D55A}".len()];
        num.parse::<f64>().ok()
    })]
    #[regex(r"[0-9]+(\.[0-9]+([eE][+-]?[0-9]+)?)?i", priority = 3, callback = |lex| {
        let s = lex.slice();
        s[..s.len() - 1].parse::<f64>().ok()
    })]
    ImagLitI(f64),

    #[regex(r"[0-9]+(\.[0-9]+([eE][+-]?[0-9]+)?)?\u{1D55B}", priority = 3, callback = |lex| {
        let s = lex.slice();
        let num = &s[..s.len() - "\u{1D55B}".len()];
        num.parse::<f64>().ok()
    })]
    #[regex(r"[0-9]+(\.[0-9]+([eE][+-]?[0-9]+)?)?j", priority = 3, callback = |lex| {
        let s = lex.slice();
        s[..s.len() - 1].parse::<f64>().ok()
    })]
    ImagLitJ(f64),

    #[regex(r"[0-9]+(\.[0-9]+([eE][+-]?[0-9]+)?)?\u{1D55C}", priority = 3, callback = |lex| {
        let s = lex.slice();
        let num = &s[..s.len() - "\u{1D55C}".len()];
        num.parse::<f64>().ok()
    })]
    #[regex(r"[0-9]+(\.[0-9]+([eE][+-]?[0-9]+)?)?k", priority = 3, callback = |lex| {
        let s = lex.slice();
        s[..s.len() - 1].parse::<f64>().ok()
    })]
    ImagLitK(f64),

    // Standalone Unicode imaginary units
    #[token("\u{1D55A}", priority = 5)]
    ImagI,
    #[token("\u{1D55B}", priority = 5)]
    ImagJ,
    #[token("\u{1D55C}", priority = 5)]
    ImagK,
    ...
    /// Invalid/unrecognized token (not generated by logos; constructed by the lexer)
    Error(String),
}
...
impl Token {
    ...
    pub fn can_start_expr(&self) -> bool {
        matches!(self,
            ...
            Token::ImagLitI(_) | Token::ImagLitJ(_) | Token::ImagLitK(_) |
            Token::ImagI | Token::ImagJ | Token::ImagK |
...
impl fmt::Display for Token {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Token::ImagLitI(x) => write!(f, "{}𝕚", x),
            Token::ImagLitJ(x) => write!(f, "{}𝕛", x),
            Token::ImagLitK(x) => write!(f, "{}𝕜", x),
            Token::ImagI => write!(f, "𝕚"),
            Token::ImagJ => write!(f, "𝕛"),
            Token::ImagK => write!(f, "𝕜"),
            Token::Error(s) => write!(f, "<invalid: {}>", s),
            _ => write!(f, "{:?}", self),
        }
    }
}
...
impl<'a> Lexer<'a> {
    ...
    fn next_inner(&mut self) -> Option<Spanned<Token>> {
        match self.inner.next() {
            Some(Ok(token)) => {
                Some(Spanned::new(token, Loc::from_span(self.inner.span())))
            }
            Some(Err(_)) => {
                let span = self.inner.span();
                let text = self.source[span.clone()].to_string();
                Some(Spanned::new(Token::Error(text), Loc::from_span(span)))
            }
            None => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_imaginary_int_unicode() {
        let mut lex = Lexer::new("4\u{1D55A}");
        assert_eq!(lex.next(), Some(Token::ImagLitI(4.0)));
    }

    #[test]
    fn test_imaginary_float_unicode() {
        let mut lex = Lexer::new("3.14\u{1D55A}");
        assert_eq!(lex.next(), Some(Token::ImagLitI(3.14)));
    }

    #[test]
    fn test_imaginary_ascii_suffix() {
        let mut lex = Lexer::new("4i 2.5j 3k");
        assert_eq!(lex.next(), Some(Token::ImagLitI(4.0)));
        assert_eq!(lex.next(), Some(Token::ImagLitJ(2.5)));
        assert_eq!(lex.next(), Some(Token::ImagLitK(3.0)));
    }

    #[test]
    fn test_standalone_imaginary_unicode() {
        let mut lex = Lexer::new("\u{1D55A} \u{1D55B} \u{1D55C}");
        assert_eq!(lex.next(), Some(Token::ImagI));
        assert_eq!(lex.next(), Some(Token::ImagJ));
        assert_eq!(lex.next(), Some(Token::ImagK));
    }

    #[test]
    fn test_complex_type_tokens() {
        let mut lex = Lexer::new("\u{2102} \u{210D}");
        assert_eq!(lex.next(), Some(Token::TyComplex));
        assert_eq!(lex.next(), Some(Token::TyQuaternion));
    }

    #[test]
    fn test_complex_type_ascii() {
        let mut lex = Lexer::new("Complex Quaternion");
        assert_eq!(lex.next(), Some(Token::TyComplex));
        assert_eq!(lex.next(), Some(Token::TyQuaternion));
    }
    ...
    fn test_complex_expression_tokens() {
        let mut lex = Lexer::new("3 + 4\u{1D55A}");
        assert_eq!(lex.next(), Some(Token::Int(3)));
        assert_eq!(lex.next(), Some(Token::Plus));
        assert_eq!(lex.next(), Some(Token::ImagLitI(4.0)));
    }
